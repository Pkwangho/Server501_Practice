...
//    public void list(Model model) {
    ...
    public String registerPost(@Valid FoodDTO foodDTO, BindingResult bindingResult,
        ...
        log.info("TodoController register post  todoDTO : " + foodDTO);
    ...
    // 그러면, 이 데이터 서버에서 용할려면, 컨트롤러에서, 받는 매개변수가 필요해요.
    //    // 자동으로이 쿼리스트링으로 넘어온 데이터 자동으로 받기.
    //
...
//            redirectAttributes.addAttribute("size",pageRequestDTO.getSize());
        ...
        FoodDTO foodDTO = foodService.getOne(tno);
        log.info("TodoController update 데이터 유무 확인 :" + foodDTO);
        //데이터 탑재. 서버 -> 웹
        model.addAttribute("todoDTO", foodDTO);
        redirectAttributes.addAttribute("page", pageRequestDTO.getPage());
        redirectAttributes.addAttribute("size", pageRequestDTO.getSize());
        return "/food/update";
    ...
    public String updateLogic(@Valid FoodDTO foodDTO, BindingResult bindingResult,
        ...
        log.info("todoDTO확인 finished의 변환 여부 확인. : " + foodDTO);
