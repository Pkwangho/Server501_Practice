                    ...
                    <div class="card-body">
                        ...
                        <form action="/user/update" method="post">
                            ...
                            <div class="input-group mb-3">
                                ...
                                <input type="text" name="uno" class="form-control" readonly
                                       value=
                                <c:out value="${userDTO.uno}"></c:out>>
                            </div>
                            <div class="input-group mb-3">
                                <span class="input-group-text">Name</span>
                                <input type="text" name="name" class="form-control" placeholder="이름을 입력해주세요"
                                       value='<c:out value="${userDTO.name}"></c:out>'>
                            </div>
                            <div class="input-group mb-3">
                                <span class="input-group-text">Password</span>
                                <input type="text" name="password" class="form-control"
                                       value=<c:out value="${userDTO.password}"></c:out>>
                            </div>

                            <div class="input-group mb-3">
                                <span class="input-group-text">DueDate</span>
                                <input type="date" name="dueDate" class="form-control"
                                       value=<c:out value="${userDTO.dueDate}"></c:out>>
                            </div>
...
<script>
    ...
    document.querySelector(".btn-primary").addEventListener("click",
        function (e) {
            ...
            self.location = "/user/update?uno=" +${userDTO.uno}
                , false
        })
    // 목록
    document.querySelector(".btn-secondary").addEventListener("click",
        function (e) {
            // 수정폼으로 가야함. 그러면, 필요한 준비물 tno 번호가 필요함
            self.location = "/user/list?${pageRequestDTO.link}"
                , false
        })
    ...
    document.querySelector(".btn-danger").addEventListener("click",
        function (e) {
            ...
            // 삭제 후에도, 검색 내역을 유지하기.
            formObj.action = "/user/delete?${pageRequestDTO.link}"
            formObj.method = "post"
    ...
    document.querySelector(".btn-primary").addEventListener("click",
        function (e) {
            ...
            formObj.action = "/user/update?${pageRequestDTO.link}"
            formObj.method = "post"
            // todoDTO 모든 멤버가 같이 전달됨.
            // tno, title, dueDate, finished, writer
            formObj.submit()
